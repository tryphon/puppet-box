...
GetOptions(
		...
		"peer-critical=i" => \(my $peer_critical_threshold = '1'),
		"peer-warning=i" => \(my $peer_warning_threshold = '2'),
		"help" => \&display_help,
);
...
} elsif (($selected_backup < 1) && ($peer_warning_threshold > 0)) {
	print_overall_health("Warning");
	print_server_list();
	exit 1;
}
...
sub display_help {
	...
	print "\t--peer-critical <num>\t-Set the critical threshold for number of peers (default:1)\n";
	print "\t--peer-warning <num>\t-Set the warning threshold for number of peers (default:2)\n";
